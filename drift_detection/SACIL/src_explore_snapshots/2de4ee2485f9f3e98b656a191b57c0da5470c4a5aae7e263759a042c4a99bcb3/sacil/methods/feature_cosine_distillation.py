...
def lucir_interclass_margin_ranking_loss(
    ...
    *,
    ...
    """Official-LUCIR-style old-vs-new margin ranking objective.

    ``cosine_scores`` must contain the unscaled normalized-classifier scores.
    For every replay row whose target is an old class, the ground-truth score
    is ranked above each of the top-k incoming-class scores by ``margin``.
    """

    if cosine_scores.ndim != 2 or targets.ndim != 1:
        raise ValueError("cosine scores must be [batch, classes] and targets [batch]")
    if cosine_scores.shape[0] != targets.shape[0]:
        raise ValueError("cosine scores and targets must share the batch size")
    if known_classes <= 0 or known_classes >= cosine_scores.shape[1]:
        raise ValueError("known_classes must split non-empty old and new classes")
    if not math.isfinite(margin) or margin < 0:
        raise ValueError("margin must be finite and non-negative")
    new_classes = cosine_scores.shape[1] - int(known_classes)
    if top_k <= 0 or top_k > new_classes:
        raise ValueError("top_k must be positive and no larger than new classes")
    if not torch.isfinite(cosine_scores).all():
        raise ValueError("cosine scores must be finite")

    old_mask = targets < int(known_classes)
    if not bool(old_mask.any()):
        return cosine_scores.sum() * 0.0
    old_targets = targets[old_mask].long()
    if bool((old_targets < 0).any()):
        raise ValueError("targets must be non-negative")
    old_scores = cosine_scores[old_mask]
    ground_truth = old_scores.gather(1, old_targets[:, None])
    hard_new = old_scores[:, int(known_classes) :].topk(
        int(top_k), dim=1
    ).values
    return F.relu(float(margin) - ground_truth + hard_new).mean()
