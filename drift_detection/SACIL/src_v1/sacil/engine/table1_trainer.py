...
SACIL_V1_HIERARCHY_SOURCES = frozenset(
    {"visual_confusion", "prototype_cosine", "random"}
)
SACIL_V1_INITIAL_DATA = frozenset({"full", "memory"})
...
def resolve_sacil_v1_options(method_config: dict[str, Any]) -> dict[str, Any]:
    """Validate the isolated SACIL-v1 hierarchy/path-loss contract."""

    raw = method_config.get("sacil_v1", {})
    if raw is None:
        raw = {}
    if not isinstance(raw, dict):
        raise ValueError("method.sacil_v1 must be a mapping")
    enabled = bool(raw.get("enabled", False))
    hierarchy_source = str(
        raw.get("hierarchy_source", "visual_confusion")
    ).lower().replace("-", "_")
    initial_data = str(raw.get("initial_hierarchy_data", "full")).lower()
    ...
    radius_quantile = float(raw.get("radius_quantile", 0.9))
    minimum_radius = float(raw.get("minimum_radius", 0.0))
    radius_slack = float(raw.get("radius_slack", 0.05))
    minimum_overlap = float(raw.get("minimum_overlap", 0.0))
    max_ratio = float(raw.get("max_conflict_leaf_ratio", 0.1))
    margin = float(raw.get("relaxation_margin", 0.05))
    margin_decay = float(raw.get("margin_ancestor_decay", 0.7))
    include_root = bool(raw.get("include_root", False))
    random_seed_offset = int(raw.get("random_seed_offset", 9107))

    if hierarchy_source not in SACIL_V1_HIERARCHY_SOURCES:
        raise ValueError(
            "method.sacil_v1.hierarchy_source must be visual_confusion, "
            "prototype_cosine, or random"
        )
    if initial_data not in SACIL_V1_INITIAL_DATA:
        raise ValueError(
            "method.sacil_v1.initial_hierarchy_data must be full or memory"
        )
    if path_scope not in {"parent_only", "all_ancestors"}:
        raise ValueError(
    ...
    if not 0.0 < radius_quantile <= 1.0:
        raise ValueError("SACIL-v1 radius_quantile must be in (0, 1]")
    if not 0.0 <= minimum_radius < math.pi:
        raise ValueError("SACIL-v1 minimum_radius must be in [0, pi)")
    if not 0.0 <= radius_slack < math.pi:
        raise ValueError("SACIL-v1 radius_slack must be in [0, pi)")
    if not 0.0 < max_ratio <= 1.0:
        raise ValueError("SACIL-v1 max_conflict_leaf_ratio must be in (0, 1]")
    if not 0.0 <= margin <= 2.0:
        raise ValueError("SACIL-v1 relaxation_margin must be in [0, 2]")
    if not 0.0 <= margin_decay <= 1.0:
        raise ValueError("SACIL-v1 margin_ancestor_decay must be in [0, 1]")
    return {
        "enabled": enabled,
        ...
        "max_conflict_leaf_ratio": max_ratio,
        "relaxation_margin": margin,
        "margin_ancestor_decay": margin_decay,
...
def geometry_preservation_component(
    geometry: AnchorGeometryLoss | SACILV1PathLoss | None,
    current_features: Tensor,
    reference_features: Tensor | None,
    replay_mask: Tensor,
    *,
    weight: float,
    original_targets: Tensor | None = None,
) -> Tensor | None:
    ...
    if (
        ...
        return None
    current = current_features[replay_mask]
    reference = reference_features[replay_mask]
    if isinstance(geometry, SACILV1PathLoss):
        if original_targets is None:
            raise ValueError("SACIL-v1 geometry requires original class IDs")
        return float(weight) * geometry(
            current,
            reference,
            original_targets[replay_mask],
        )
    return float(weight) * geometry(current, reference)
...
class UnifiedTable1Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        self.sacil_v1_options = resolve_sacil_v1_options(
            self.config["method"]
        )
        if self.sacil_v1_options["enabled"]:
            if self.method not in {"icarl", "afc"}:
                raise ValueError(
                    "SACIL-v1 currently supports the controlled iCaRL or AFC "
                    "substrate"
                )
            if self.geometry_mode == "none":
                raise ValueError(
                    "SACIL-v1 requires a non-none geometry_mode"
                )
            if self.geometry_options["anchor_frame"] != "co_moving":
                raise ValueError(
                    "SACIL-v1 requires geometry.anchor_frame=co_moving"
                )
        ...
        self.sacil_v1_pending_plan: ConflictPlan | None = None
        self.sacil_v1_last_plan: ConflictPlan | None = None
        self.sacil_v1_insertion_log: tuple[dict[str, object], ...] = ()
        ...
        dump_json(
            {
                ...
                "sacil_v1_options": self.sacil_v1_options,
    ...
    def _validate_base_checkpoint(self, checkpoint: dict[str, Any]) -> None:
        if checkpoint.get("framework") != "sacil-unified-v1":
            raise ValueError("base checkpoint is not from the SACIL-v1 runner")
    ...
    def _bootstrap_from_base_checkpoint(self) -> dict[str, Any]:
        ...
        source_hierarchy = checkpoint.get("config", {}).get("method", {}).get(
            "hierarchy", {}
        )
        ...
        if self.geometry_mode == "none":
            ...
            not self.sacil_v1_options["enabled"]
            and has_artifacts
        ...
        record.update(
            ...
            sacil_v1_options=copy.deepcopy(self.sacil_v1_options),
    ...
    def _run_session(self, session_id: int) -> dict[str, Any]:
        ...
        record = {
            ...
            "geometry_options": copy.deepcopy(self.geometry_options),
            "sacil_v1_options": copy.deepcopy(self.sacil_v1_options),
            "sacil_v1_conflict_plan": (
                None
                if self.sacil_v1_last_plan is None
                else self.sacil_v1_last_plan.state_dict()
            ),
            "sacil_v1_insertions": list(self.sacil_v1_insertion_log),
            "casper_options": copy.deepcopy(self.casper_options),
    ...
    ) -> AnchorGeometryLoss | SACILV1PathLoss | None:
        if self.geometry_mode == "none" or session_id == 0:
            return None
        if self.sacil_v1_options["enabled"]:
            return self._prepare_sacil_v1_geometry(session_id, teacher)
        if (
            teacher is None
    ...
    def _prepare_sacil_v1_geometry(
        self,
        session_id: int,
        teacher: nn.Module | None,
    ) -> SACILV1PathLoss:
        """Freeze the old tree and route only the incoming conflict subtree."""

        if teacher is None or self.sacil_tree is None:
            raise RuntimeError("SACIL-v1 incremental session lacks old state")
        old_class_ids = self.sacil_tree.class_order
        old_collection = collect_features(
            teacher,
            self._memory_loader(session_id, augment=False),
            self.device,
        )
        old_prototypes = PrototypeBank(
            old_class_ids,
            compute_prototypes(
                old_collection.features,
                old_collection.original_targets,
        ...
        reference_anchors = HierarchicalAnchorBank.from_tree(
            old_prototypes, self.sacil_tree
        )
        node_radii = compute_internal_node_radii(
            old_collection.features,
            old_collection.original_targets,
            reference_anchors,
            self.sacil_tree,
            quantile=float(self.sacil_v1_options["radius_quantile"]),
            minimum_radius=float(self.sacil_v1_options["minimum_radius"]),
        )
        incoming_collection = collect_features(
            ...
            self._loader(
                ...
                shuffle=False,
                session_id=session_id + 4100,
            ),
            self.device,
        )
        new_class_ids = self.protocol.classes_for_session(session_id)
        ...
        plan = plan_conflict_relaxation(
            ...
            node_radii,
            relaxation_mode=str(
                self.sacil_v1_options["relaxation_mode"]
            ),
            path_scope=str(self.sacil_v1_options["path_scope"]),
            radius_slack=float(self.sacil_v1_options["radius_slack"]),
            minimum_overlap=float(
                self.sacil_v1_options["minimum_overlap"]
            ),
            max_conflict_leaf_ratio=float(
                self.sacil_v1_options["max_conflict_leaf_ratio"]
            ),
            relaxation_margin=float(
                self.sacil_v1_options["relaxation_margin"]
            ),
            margin_ancestor_decay=float(
                self.sacil_v1_options["margin_ancestor_decay"]
            ),
            include_root=bool(self.sacil_v1_options["include_root"]),
        )
        self.sacil_v1_pending_plan = plan
        self.sacil_v1_last_plan = plan
        return SACILV1PathLoss(
            reference_anchors,
            self.sacil_tree,
            plan,
            path_scope=str(self.sacil_v1_options["path_scope"]),
    ...
    def _refresh_geometry_current_anchors(
        self,
        session_id: int,
        geometry: AnchorGeometryLoss | SACILV1PathLoss,
    ) -> None:
    ...
    def _train_session(
        ...
        for epoch in progress:
            ...
            for batch_index, batch in enumerate(loader):
                ...
                original_targets = batch["original_target"].to(
        ...
        if isinstance(geometry, SACILV1PathLoss):
            result["sacil_v1_geometry"] = geometry.diagnostics()
        return result
    ...
    def _build_sacil_artifacts(self, session_id: int) -> None:
        if self.sacil_v1_options["enabled"]:
            self._build_sacil_v1_artifacts(session_id)
            return
        if self.model is None:
    ...
    def _build_sacil_v1_artifacts(self, session_id: int) -> None:
        """Construct S0 once, then grow the tree by deterministic insertion."""

        if self.model is None:
            raise RuntimeError("SACIL-v1 artifacts have no model")
        class_ids = self.protocol.seen_classes(session_id)
        if session_id == 0:
            if self.sacil_v1_options["initial_hierarchy_data"] == "full":
                dataset = self.data.train_eval_dataset_for_classes(
                    class_ids,
                    samples_per_class=self.debug_train_samples_per_class,
                )
            ...
            source = self.sacil_v1_options["hierarchy_source"]
            if source == "visual_confusion":
                hierarchy = self.config["method"].get("hierarchy", {})
                affinity = symmetric_affinity(
                    cosine_soft_confusion(
                        collection.features,
                        collection.targets,
                        values,
                        temperature=float(
                            hierarchy.get("taxonomy_temperature", 0.2)
                        ),
                    )
                )
            elif source == "prototype_cosine":
                ...
                generator = make_generator(
                    self.seed
                    + int(self.sacil_v1_options["random_seed_offset"])
                )
                affinity = torch.rand(
                    len(class_ids), len(class_ids), generator=generator
                )
                affinity = 0.5 * (affinity + affinity.t())
                affinity.fill_diagonal_(0.0)
            tree = GriffinPeronaGreedy().build(class_ids, affinity)
            prototypes = PrototypeBank(class_ids, values)
            self.sacil_v1_insertion_log = ()
        else:
            if self.sacil_tree is None or self.sacil_v1_pending_plan is None:
                raise RuntimeError(
                    "SACIL-v1 online insertion lacks the old tree or plan"
                )
            tree, insertion_log = insert_planned_classes(
                self.sacil_tree, self.sacil_v1_pending_plan
            )
            if tree.class_order != class_ids:
                raise RuntimeError(
                    "SACIL-v1 inserted tree differs from protocol class order"
                )
            collection = collect_features(
                self.model,
                self._memory_loader(session_id, augment=False),
            ...
            self.sacil_v1_insertion_log = insertion_log
            self.sacil_v1_last_plan = self.sacil_v1_pending_plan
            self.sacil_v1_pending_plan = None
        self.sacil_tree = tree
        self.sacil_prototypes = prototypes
        self.sacil_anchors = HierarchicalAnchorBank.from_tree(prototypes, tree)
        dump_json(
            tree.state_dict(),
            self.run_dir / f"tree_session_{session_id:02d}.json",
        )
        if session_id > 0 and self.sacil_v1_last_plan is not None:
            dump_json(
                {
                    "plan": self.sacil_v1_last_plan.state_dict(),
                    "insertions": list(self.sacil_v1_insertion_log),
                },
                self.run_dir
                / f"conflict_plan_session_{session_id:02d}.json",
            )
