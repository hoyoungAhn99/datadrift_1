...
class AFCIncrementalNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        backbone: str = "rebuffi_importance_resnet32",
    ) -> None:
        super().__init__()
        if backbone == "rebuffi_importance_resnet32":
            self.backbone: AFCResNet32 | AFCResNet18 = afc_resnet32()
        elif backbone == "afc_resnet18_imagenet_importance":
            self.backbone = afc_resnet18()
        else:
            raise ValueError(f"unsupported AFC backbone: {backbone}")
