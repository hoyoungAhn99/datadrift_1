...
def validate_imagenet100_b50_inc5_config(config: dict[str, Any]) -> None:
    """Reject nuisance-variable drift in the CMPT ImageNet-100 suite."""

    data = config.get("data", {})
    model = config.get("model", {})
    memory = config.get("memory", {})
    evaluation = config.get("evaluation", {})
    normalized = str(data.get("name", "")).lower().replace("-", "").replace(
        "_", ""
    )
    if normalized != "imagenet100":
        raise ValueError("ImageNet CMPT contract requires ImageNet-100")
    if not str(data.get("protocol", "")).replace("\\", "/").endswith(
        "imagenet100_b50_inc5_afc_order1.json"
    ):
        raise ValueError(
            "ImageNet CMPT contract requires the fixed AFC/PODNet class order"
        )
    if "resnet18" not in str(model.get("backbone", "")).lower():
        raise ValueError("ImageNet CMPT contract requires ResNet-18")
    if str(memory.get("mode", "")) != "per_class":
        raise ValueError("ImageNet CMPT contract requires per-class memory")
    if int(memory.get("exemplars_per_class", -1)) != 20:
        raise ValueError("ImageNet CMPT contract requires 20 exemplars/class")
    if str(memory.get("selection", "")) != "icarl_herding":
        raise ValueError("ImageNet CMPT contract requires shared iCaRL herding")
    if str(evaluation.get("classifier", "")).lower() != "nme":
        raise ValueError("ImageNet CMPT contract requires NME evaluation")


def validate_imagenet100_b50_inc5_protocol(protocol: Any) -> None:
    sessions = [protocol.session(index) for index in range(protocol.num_sessions)]
    sizes = [session.size for session in sessions]
    if sizes != [50] + [5] * 10:
        raise ValueError("ImageNet CMPT contract requires B50 + 10x5")
