...
def restore_rng_state(
    ...
    *,
    ...
    Shared S0 checkpoints can be produced on ``cuda:1`` and continued on
    ``cuda:0`` or under a one-device ``CUDA_VISIBLE_DEVICES`` mask. Restoring
    every serialized device with ``set_rng_state_all`` would then index past
    the visible generator tuple. Instead, transplant the source device state
    to the requested target logical device.
    """
    ...
    if target_index is not None:
        if not 0 <= target_index < torch.cuda.device_count():
            raise ValueError(f"target CUDA device {target_index} is not visible")
        if source_index is None:
            source_index = min(target_index, len(cuda_states) - 1)
        if not 0 <= source_index < len(cuda_states):
            raise ValueError(
                f"checkpoint has no CUDA RNG state for source device {source_index}"
            )
        torch.cuda.set_rng_state(cuda_states[source_index], target_index)
        return
    for index, cuda_state in enumerate(cuda_states[: torch.cuda.device_count()]):
