...
def resolve_feature_cosine_distillation_options(
    ...
    margin_raw = raw.get("margin_ranking")
    if margin_raw is not None and not isinstance(margin_raw, dict):
        raise ValueError("feature cosine margin_ranking must be a mapping")
    margin_enabled = bool(
        isinstance(margin_raw, dict) and margin_raw.get("enabled", False)
    )
    if not enabled:
        if routing_enabled or margin_enabled:
            raise ValueError(
                "feature cosine routing/margin requires feature distillation"
            )
        return {"enabled": False}
    if str(method_name).lower() != "icarl":
    ...
    sample_scope = str(raw.get("sample_scope", "all")).lower().replace("-", "_")
    combine_old_logit_kd = bool(raw.get("combine_old_logit_kd", False))
    if not math.isfinite(weight) or weight < 0:
    ...
    if adaptive_mode not in {
        ...
        raise ValueError(
            "feature cosine adaptive_mode must be none, new_over_old, "
            "old_over_new, or seen_over_new"
        )
    ...
    if sample_scope not in {"all", "replay_only"}:
        raise ValueError("feature cosine sample_scope must be all or replay_only")
    if margin_enabled and training_classifier != "normalized_cosine":
        raise ValueError(
            "LUCIR margin ranking requires normalized_cosine classifier"
        )
    options = {
        "enabled": True,
        "lambda": weight,
        ...
        "combine_old_logit_kd": combine_old_logit_kd,
    ...
    if margin_raw is not None:
        margin_options = {"enabled": margin_enabled}
        if margin_enabled:
            margin = float(margin_raw.get("margin", 0.5))
            top_k = int(margin_raw.get("top_k", 2))
            margin_weight = float(margin_raw.get("weight", 1.0))
            if not math.isfinite(margin) or margin < 0:
                raise ValueError("LUCIR margin must be finite and non-negative")
            if top_k <= 0:
                raise ValueError("LUCIR margin top_k must be positive")
            if not math.isfinite(margin_weight) or margin_weight < 0:
                raise ValueError(
                    "LUCIR margin weight must be finite and non-negative"
                )
            margin_options.update(
                {
                    "margin": margin,
                    "top_k": top_k,
        ...
        options["margin_ranking"] = margin_options
    return options


def resolve_replay_batching_options(
    method_name: str, method_config: dict[str, Any]
) -> dict[str, Any]:
    """Resolve opt-in dual-stream replay used by SACIL exploration.

    The total number of optimizer steps is kept equal to the ordinary shuffled
    loader.  Only the old/new composition of each incremental batch changes.
    A conflict multiplier above one is hierarchy-conditioned and therefore
    requires an enabled BGS reference.
    """

    raw = method_config.get("replay_batching", {})
    if raw is None:
        raw = {}
    if not isinstance(raw, dict):
        raise ValueError("method.replay_batching must be a mapping")
    enabled = bool(raw.get("enabled", False))
    if not enabled:
        return {"enabled": False}
    supported_methods = {"icarl", "fgp"}
    normalized_method = str(method_name).lower()
    if normalized_method not in supported_methods:
        raise ValueError(
            "dual-stream replay batching requires an iCaRL or FGP substrate"
        )
    fraction_mode = str(raw.get("fraction_mode", "fixed")).lower()
    replay_fraction = float(raw.get("replay_fraction", 0.5))
    conflict_multiplier = float(raw.get("conflict_multiplier", 1.0))
    if fraction_mode not in {"fixed", "class_uniform"}:
        raise ValueError("replay fraction_mode must be fixed or class_uniform")
    ...
    if not math.isfinite(conflict_multiplier) or conflict_multiplier <= 0.0:
        raise ValueError("conflict_multiplier must be finite and positive")
    if conflict_multiplier != 1.0:
        bgs = method_config.get("boundary_graph_surgery", {})
        if not isinstance(bgs, dict) or not bool(bgs.get("enabled", False)):
            raise ValueError(
                "hierarchy-conditioned replay batching requires enabled BGS"
            )
    return {
        "enabled": True,
        ...
        "class_balanced_replay": True,
        "preserve_optimizer_steps": True,
        "implementation": "hierarchy_conditioned_dual_stream_replay_v1",
    }


def resolve_classification_reweighting_options(
    method_name: str, method_config: dict[str, Any]
) -> dict[str, Any]:
    raw = method_config.get("classification_reweighting", {})
    if raw is None:
        raw = {}
    if not isinstance(raw, dict):
        raise ValueError("method.classification_reweighting must be a mapping")
    enabled = bool(raw.get("enabled", False))
    if not enabled:
        return {"enabled": False}
    if str(method_name).lower() != "icarl":
        raise ValueError("classification reweighting requires iCaRL")
    old_weight = float(raw.get("old_weight", 1.0))
    new_weight = float(raw.get("new_weight", 1.0))
    if not math.isfinite(old_weight) or old_weight <= 0.0:
        raise ValueError("classification old_weight must be finite and positive")
    if not math.isfinite(new_weight) or new_weight <= 0.0:
        raise ValueError("classification new_weight must be finite and positive")
    return {
        "enabled": True,
        "old_weight": old_weight,
        "new_weight": new_weight,
        "normalization": "sample_weight_sum",
        "implementation": "dual_stream_group_weighted_ce_v1",
    }


def resolve_prototype_transport_options(
    method_name: str, method_config: dict[str, Any]
) -> dict[str, Any]:
    raw = method_config.get("prototype_transport", {})
    if raw is None:
        raw = {}
    if not isinstance(raw, dict):
        raise ValueError("method.prototype_transport must be a mapping")
    enabled = bool(raw.get("enabled", False))
    if not enabled:
        return {"enabled": False}
    if str(method_name).lower() != "icarl":
        raise ValueError("prototype transport requires iCaRL")
    mode = str(raw.get("mode", "global")).lower()
    sigma = float(raw.get("sigma", 0.2))
    if mode not in {
        "global",
        "class",
        "hierarchy_parent",
        ...
        raise ValueError(
            "prototype transport mode must be global, class, "
            "hierarchy_parent, procrustes_global, procrustes_rigid, "
            "or procrustes_rigid_flip"
        )
    if not math.isfinite(sigma) or sigma <= 0.0:
        raise ValueError("prototype transport sigma must be finite and positive")
    return {
        "enabled": True,
        "mode": mode,
        "sigma": sigma,
        "old_evaluation_source": "transported_full_introduction_mean",
        "new_evaluation_source": "current_herding_memory_mean",
        "horizontal_flip_consistent": mode == "procrustes_rigid_flip",
        "implementation": "co_moving_prototype_transport_v2",
    }


def resolve_prototype_consolidation_options(
    method_name: str, method_config: dict[str, Any]
) -> dict[str, Any]:
    raw = method_config.get("prototype_consolidation", {})
    if raw is None:
        raw = {}
    if not isinstance(raw, dict):
        raise ValueError("method.prototype_consolidation must be a mapping")
    if not bool(raw.get("enabled", False)):
        return {"enabled": False}
    if str(method_name).lower() != "icarl":
        raise ValueError("prototype consolidation requires iCaRL")
    weight = float(raw.get("lambda", 1.0))
    conflict_weight = float(raw.get("conflict_weight", 1.0))
    outside_weight = float(raw.get("outside_weight", 1.0))
    for name, value in (("lambda", weight), ("conflict_weight", conflict_weight), ("outside_weight", outside_weight)):
        if not math.isfinite(value) or value < 0:
            raise ValueError(f"prototype consolidation {name} must be finite and non-negative")
    if conflict_weight != outside_weight:
        bgs = method_config.get("boundary_graph_surgery", {})
        if not isinstance(bgs, dict) or not bool(bgs.get("enabled", False)):
            raise ValueError("hierarchy-weighted prototype consolidation requires BGS")
    return {
        "enabled": True,
        "lambda": weight,
        "conflict_weight": conflict_weight,
        "outside_weight": outside_weight,
        "implementation": "teacher_memory_prototype_consolidation_v1",
    }
...
def resolve_casper_options(
    ...
    CaSpeR is a plug-in rather than a standalone learner. ``method=casper``
    denotes the author repository's ``ICarl(CasperModel)`` adapter, including
    its BCE target construction and explicit L2 term. The controlled
    comparison instead uses ``method=icarl`` with
...
def base_recipe_signature(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(method.get("name", "")).lower() == "icarl":
        ...
        method.pop("replay_batching", None)
        method.pop("prototype_consolidation", None)
        method.pop("classification_reweighting", None)
        method.pop("prototype_transport", None)
        ...
        for _ in range(self.batches):
            ...
            for offset, class_id in enumerate(selected.tolist()):
                ...
                choices = rng.choice(
                    candidates,
                    size=count,
                    # CaSpeR's Buffer.get_balanced_data samples without
                    # replacement whenever a selected class contains enough
                    # exemplars.  Falling back to replacement only preserves
                    # the author's size-clamping behavior for a deliberately
                    # undersized debug memory.
...
class ReplayBalancedBatchSampler(Sampler[list[int]]):
    """Deterministic fixed-ratio new/replay batches with class-balanced replay.

    Replay class probabilities are uniform for the matched control.  The SACIL
    candidate multiplies probabilities for classes in learned conflict branches,
    while keeping the total replay count and optimizer-step count unchanged.
    """
    ...
    def __init__(
        ...
        *,
        ...
        batches: int,
        seed: int,
        conflict_labels: set[int] | None = None,
        ...
        self.conflict_labels = frozenset(int(v) for v in (conflict_labels or set()))
        self.conflict_multiplier = float(conflict_multiplier)
        if self.new_size <= 0 or not replay_labels:
            raise ValueError("dual-stream sampler requires new and replay samples")
        if self.batch_size < 2 or self.batches <= 0:
            raise ValueError("dual-stream sampler dimensions must be positive")
        if not 0.0 < self.replay_fraction < 1.0:
            raise ValueError("dual-stream replay fraction must be in (0, 1)")
        self.replay_per_batch = int(round(self.batch_size * self.replay_fraction))
        self.replay_per_batch = min(
            self.batch_size - 1, max(1, self.replay_per_batch)
        )
        self.new_per_batch = self.batch_size - self.replay_per_batch
        positions: dict[int, list[int]] = defaultdict(list)
        for offset, label in enumerate(replay_labels):
            positions[int(label)].append(self.new_size + int(offset))
        self.positions = dict(sorted(positions.items()))
        if not self.conflict_labels <= set(self.positions):
            raise ValueError("conflict labels must be replay classes")
        classes = tuple(self.positions)
        weights = np.asarray(
            [
                self.conflict_multiplier if label in self.conflict_labels else 1.0
                for label in classes
            ],
            dtype=np.float64,
        )
        if not np.isfinite(weights).all() or bool((weights <= 0).any()):
            raise ValueError("replay class weights must be finite and positive")
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        for _ in range(self.batches):
            new_positions = rng.choice(
                self.new_size,
                size=self.new_per_batch,
                replace=self.new_per_batch > self.new_size,
            ).tolist()
            replay_classes = rng.choice(
                self.classes,
                size=self.replay_per_batch,
                replace=True,
                p=self.class_probabilities,
            )
            replay_positions = [
                int(rng.choice(self.positions[int(label)]))
                for label in replay_classes.tolist()
            ]
            batch = [int(v) for v in new_positions] + replay_positions
            rng.shuffle(batch)
            yield batch
...
class UnifiedTable1Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        dataset_name = str(
            self.config.get("data", {}).get("name", "")
        ).lower().replace("-", "").replace("_", "")
        if dataset_name == "cifar100":
            validate_annotation1_config(self.config)
        elif dataset_name == "imagenet100":
            validate_imagenet100_b50_inc5_config(self.config)
        else:
            raise ValueError(f"unsupported unified dataset: {dataset_name}")
        ...
        self.replay_batching_options = resolve_replay_batching_options(
            self.method, self.config["method"]
        )
        self.classification_reweighting_options = (
            resolve_classification_reweighting_options(
                self.method, self.config["method"]
            )
        )
        self.prototype_transport_options = resolve_prototype_transport_options(
            self.method, self.config["method"]
        )
        self.prototype_consolidation_options = (
        ...
        if (
            ...
            and self.icarl_kd_weight != 0.0
            and not self.feature_cosine_distillation_options.get(
                "combine_old_logit_kd", False
            )
        ):
            raise ValueError(
                "cosine feature control replaces old-logit KD unless "
                "combine_old_logit_kd is explicitly enabled"
            )
        if (
            self.feature_cosine_distillation_options["enabled"]
        ...
        runtime_options = config.get("runtime", {})
        ...
        self.gpu_throttle_seconds = (
            throttle_ms / 1000.0 if self.device.type == "cuda" else 0.0
        )
        ...
        if dataset_name == "cifar100":
            ...
            validate_imagenet100_b50_inc5_protocol(self.protocol)
        ...
        self._last_replay_batching_stats: dict[str, Any] = {"enabled": False}
        self._prototype_consolidation_anchors: Tensor | None = None
        self.transported_class_means: Tensor | None = None
        self._last_prototype_transport_stats: dict[str, Any] = {
            "enabled": False
        }
        ...
        dump_json(
            {
                ...
                "method_contract": self.method_contract_payload,
                ...
                "replay_batching_options": self.replay_batching_options,
                "classification_reweighting_options": (
                    self.classification_reweighting_options
                ),
                "prototype_transport_options": self.prototype_transport_options,
    ...
    @property
    def method_contract_payload(self) -> dict[str, Any]:
        payload = self.method_contract.as_dict()
        payload["backbone_family"] = str(
            self.config.get("model", {}).get("backbone", "unspecified")
        )
        return payload
    ...
    def _new_model(self, num_classes: int) -> nn.Module:
        ...
        if self.method == "afc":
            return AFCIncrementalNet(
                ...
                backbone=str(
                    model.get("backbone", "rebuffi_importance_resnet32")
                ),
            )
        ...
        if self.method == "fgp":
            return FGPIncrementalNet(
                num_classes,
                backbone=str(
                    model.get("backbone", "fgp_resnet32_no_last_relu")
                ),
            )
    ...
    def _training_batch_sampler(
        self,
        session_id: int,
        dataset,
        old_indices: list[int],
    ) -> ReplayBalancedBatchSampler | None:
        """Build the fixed-step dual-stream sampler for incremental sessions."""

        self._last_replay_batching_stats = {"enabled": False}
        options = self.replay_batching_options
        if session_id == 0 or not options["enabled"]:
            return None
        if not isinstance(dataset, ConcatDataset) or len(dataset.datasets) != 2:
            raise TypeError("dual-stream replay expects [new, replay] ConcatDataset")
        new_size = len(dataset.datasets[0])
        replay_labels = [
            int(self.data.train_aug.targets[int(index)]) for index in old_indices
        ]
        conflict_labels: set[int] = set()
        multiplier = float(options["conflict_multiplier"])
        if multiplier != 1.0:
            if self.bgs_reference is None:
                raise RuntimeError(
                    "hierarchy-conditioned replay requires a prepared BGS reference"
                )
            conflict_labels = {
                int(class_id)
                for class_id, region in zip(
                    self.bgs_reference.old_original_ids,
            ...
            if not conflict_labels:
                raise RuntimeError("hierarchy-conditioned replay found no conflict class")
        batch_size = int(self.config["training"].get("batch_size", 128))
        batches = math.ceil(len(dataset) / batch_size)
        replay_fraction = float(options["replay_fraction"])
        if options.get("fraction_mode", "fixed") == "class_uniform":
            known = self.protocol.session(session_id).start
            total = self.protocol.session(session_id).stop
            replay_fraction = float(known) / float(total)
        sampler = ReplayBalancedBatchSampler(
            ...
            replay_fraction=replay_fraction,
            batches=batches,
            seed=self.seed * 1000 + session_id + 17000,
            conflict_labels=conflict_labels,
            conflict_multiplier=multiplier,
        )
        self._last_replay_batching_stats = {
            "enabled": True,
            "batches": batches,
            "batch_size": batch_size,
            "new_per_batch": sampler.new_per_batch,
            "replay_per_batch": sampler.replay_per_batch,
            "old_class_count": len(sampler.positions),
            "conflict_class_count": len(conflict_labels),
            "conflict_class_ratio": len(conflict_labels) / len(sampler.positions),
            "conflict_multiplier": multiplier,
            "fraction_mode": options.get("fraction_mode", "fixed"),
            "realized_replay_fraction": sampler.replay_per_batch / batch_size,
            "preserved_plain_loader_batches": batches,
        }
        return sampler

    def _prepare_prototype_consolidation(
        self, session_id: int, teacher: nn.Module | None
    ) -> None:
        self._prototype_consolidation_anchors = None
        if session_id == 0 or not self.prototype_consolidation_options["enabled"]:
            return
        if teacher is None:
            raise RuntimeError("prototype consolidation requires an old teacher")
        if self.bgs_reference is not None:
            anchors = self.bgs_reference.anchor_state["leaf_anchors"]
        else:
            ...
            anchors = compute_prototypes(
                collection.features,
                collection.original_targets,
                old_ids,
            )
        self._prototype_consolidation_anchors = (
            F.normalize(anchors.float(), dim=1).to(self.device)
        )
    ...
    def _bootstrap_from_base_checkpoint(self) -> dict[str, Any]:
        ...
        if (
            self.prototype_transport_options.get("enabled", False)
            and self.transported_class_means is None
        ):
            # The shared legacy S0 checkpoint predates prototype transport.
            # Materialize the statistic that would have been stored at the end
            # of S0; it does not update model weights or memory.
            self._initialize_full_session_prototypes(0)
    ...
    def _run_session(self, session_id: int) -> dict[str, Any]:
        ...
        self._prepare_prototype_consolidation(session_id, teacher)
        train_dataset = self._training_dataset(session_id, old_indices)
        training_batch_sampler = self._training_batch_sampler(
            session_id, train_dataset, old_indices
        )
        train_loader = self._loader(
            train_dataset,
            shuffle=training_batch_sampler is None,
            session_id=session_id,
        ...
        prototype_transport = self._update_prototype_transport(
            session_id, teacher
        )
        ...
        record = {
            ...
            "bgs_options": copy.deepcopy(self.bgs_options),
            "replay_batching_options": copy.deepcopy(
                self.replay_batching_options
            ),
            "replay_batching_diagnostics": copy.deepcopy(
                self._last_replay_batching_stats
            ),
            "classification_reweighting_options": copy.deepcopy(
                self.classification_reweighting_options
            ),
            "prototype_transport_options": copy.deepcopy(
                self.prototype_transport_options
            ),
            "prototype_transport": copy.deepcopy(prototype_transport),
            "prototype_consolidation_options": copy.deepcopy(
                self.prototype_consolidation_options
            ),
            "bgs_reference_path": (
                None
        ...
        for epoch in progress:
            ...
            for batch_index, batch in enumerate(loader):
                ...
                if projected_insertion:
                    ...
                    loss.backward()
                optimizer.step()
                if self.gpu_throttle_seconds > 0.0:
                    torch.cuda.synchronize(self.device)
                    time.sleep(self.gpu_throttle_seconds)
                optimizer_step += 1
                ...
                "feature_cosine_effective_weight": (
                    ...
                    * (
                        ...
                        if self.feature_cosine_distillation_options[
                            ...
                            float(self.protocol.session(session_id).stop)
                            / float(self.protocol.session(session_id).size)
                        )
                        if self.feature_cosine_distillation_options[
                            "adaptive_mode"
    ...
    def _loss_components(
        ...
        if isinstance(self.model, ExpandableLinearNet):
            ...
            if self.method == "icarl":
                ...
                if reweighting.get("enabled", False) and known > 0:
                    per_sample_ce = F.cross_entropy(
                        classification_logits, targets, reduction="none"
                    )
                    classification_weights = torch.where(
                        replay_mask,
                        torch.full_like(
                            per_sample_ce, float(reweighting["old_weight"])
                        ),
                        torch.full_like(
                            per_sample_ce, float(reweighting["new_weight"])
                        ),
                    )
                    classification = (
                        per_sample_ce * classification_weights
                    ).sum() / classification_weights.sum()
                else:
                    classification = F.cross_entropy(
                        classification_logits, targets
                    )
                components = {"classification": classification}
                margin_options = feature_cosine_options.get(
                    "margin_ranking", {"enabled": False}
                )
                if margin_options.get("enabled", False) and known > 0:
                    raw_cosine_scores = classification_logits / float(
                        feature_cosine_options["cosine_scale"]
                    )
                    components["interclass_margin"] = float(
                        margin_options["weight"]
                    ) * lucir_interclass_margin_ranking_loss(
                        raw_cosine_scores,
                        targets,
                        known_classes=known,
                        margin=float(margin_options["margin"]),
                        top_k=int(margin_options["top_k"]),
                    )
                ...
                if (
                    ...
                    if feature_cosine_options["adaptive_mode"] == "new_over_old":
                        ...
                        feature_cosine_options["adaptive_mode"]
                        == "seen_over_new"
                    ):
                        # Exact LUCIR ImageNet adaptation: lambda is scaled by
                        # sqrt((old + new) / new).
                        feature_weight *= math.sqrt(
                            float(known + new_count) / float(new_count)
                        )
                    sample_weights = None
                    ...
                    elif feature_cosine_options.get("sample_scope", "all") == "replay_only":
                        sample_weights = replay_mask.to(
                ...
                prototype_options = getattr(
                    self,
                    "prototype_consolidation_options",
                    {"enabled": False},
                )
                if (
                    prototype_options["enabled"]
                    and reference is not None
                    and bool(replay_mask.any())
                ):
                    anchors = self._prototype_consolidation_anchors
                    if anchors is None or anchors.shape[0] != known:
                        raise RuntimeError(
                            "prototype consolidation anchor bank is incompatible"
                        )
                    old_targets = targets[replay_mask].long()
                    current = F.normalize(output.features[replay_mask], dim=1)
                    per_sample = 1.0 - (
                        current * anchors[old_targets]
                    ).sum(dim=1)
                    conflict_weight = float(prototype_options["conflict_weight"])
                    outside_weight = float(prototype_options["outside_weight"])
                    if conflict_weight != outside_weight:
                        if self.bgs_reference is None:
                            raise RuntimeError(
                                "hierarchy prototype consolidation lacks BGS reference"
                            )
                        conflict_lookup = torch.tensor(
                            [
                                region is not None
                                for region in self.bgs_reference.sample_region_ids
                            ],
                            dtype=torch.bool,
                            device=self.device,
                        )
                        weights = torch.full_like(per_sample, outside_weight)
                        weights[conflict_lookup[old_targets]] = conflict_weight
                        proto_loss = (weights * per_sample).sum() / weights.sum().clamp_min(1e-12)
                    else:
                        proto_loss = per_sample.mean()
                    components["prototype_consolidation"] = (
                        float(prototype_options["lambda"]) * proto_loss
                    )
    ...
    def _full_session_prototypes(self, session_id: int) -> Tensor:
        if self.model is None:
            raise RuntimeError("full-session prototypes require a model")
        class_ids = self.protocol.classes_for_session(session_id)
        ...
        original_targets = collection.original_targets
        if (
            self.prototype_transport_options.get("enabled", False)
            and self.prototype_transport_options.get("mode")
            == "procrustes_rigid_flip"
        ):
            ...
            original_targets = torch.cat(
                [original_targets, flipped.original_targets], dim=0
            )
    ...
    def _initialize_full_session_prototypes(self, session_id: int) -> None:
        values = self._full_session_prototypes(session_id)
        expected = self.protocol.session(session_id).stop
        if session_id != 0 or values.shape[0] != expected:
            raise RuntimeError("legacy transport initialization requires S0")
        self.transported_class_means = values
        self._last_prototype_transport_stats = {
            "enabled": True,
            "mode": str(self.prototype_transport_options["mode"]),
            "initialized_from_full_session": 0,
            "legacy_shared_base_materialization": True,
            "prototype_count": int(values.shape[0]),
        }

    def _update_prototype_transport(
        self, session_id: int, teacher: nn.Module | None
    ) -> dict[str, Any]:
        options = self.prototype_transport_options
        if not options.get("enabled", False):
            self._last_prototype_transport_stats = {"enabled": False}
            return copy.deepcopy(self._last_prototype_transport_stats)
        if self.model is None:
            raise RuntimeError("prototype transport requires a model")
        new_prototypes = self._full_session_prototypes(session_id)
        if session_id == 0:
            self.transported_class_means = new_prototypes
            self._last_prototype_transport_stats = {
                "enabled": True,
                "mode": str(options["mode"]),
                "initialized_from_full_session": 0,
                "legacy_shared_base_materialization": False,
                "prototype_count": int(new_prototypes.shape[0]),
            }
            return copy.deepcopy(self._last_prototype_transport_stats)
        known = self.protocol.session(session_id).start
        if teacher is None or self.transported_class_means is None:
            raise RuntimeError("incremental prototype transport lacks old state")
        if self.transported_class_means.shape[0] != known:
            raise RuntimeError("transported prototype count does not match old classes")

        memory_loader = self._memory_loader(session_id, augment=False)
        old_collection = collect_features(teacher, memory_loader, self.device)
        current_collection = collect_features(
            self.model, memory_loader, self.device
        )
        if not torch.equal(old_collection.indices, current_collection.indices):
            raise RuntimeError("prototype transport feature rows are misaligned")
        class_ids = self.protocol.seen_classes(session_id - 1)
        tree = None
        if options["mode"] == "hierarchy_parent":
            teacher_prototypes = compute_prototypes(
                old_collection.features,
                old_collection.original_targets,
                class_ids,
            )
        ...
        if options["mode"] in {
            ...
            old_transport_features = old_collection.features
            current_transport_features = current_collection.features
            if options["mode"] == "procrustes_rigid_flip":
                old_flipped = collect_features(
                    teacher,
                    memory_loader,
                    self.device,
                    horizontal_flip=True,
                )
                current_flipped = collect_features(
                    self.model,
                    memory_loader,
                    self.device,
                    horizontal_flip=True,
                )
                if not torch.equal(old_flipped.indices, current_flipped.indices):
                    raise RuntimeError("transport flip rows are misaligned")
                if not torch.equal(old_collection.indices, old_flipped.indices):
                    raise RuntimeError("regular/flip transport rows are misaligned")
                old_transport_features = torch.cat(
                    [old_transport_features, old_flipped.features], dim=0
                )
                current_transport_features = torch.cat(
                    [current_transport_features, current_flipped.features], dim=0
                )
            if options["mode"] == "procrustes_global":
                moved, _, residual = orthogonal_procrustes_transport(
                    self.transported_class_means,
                    old_transport_features,
                    current_transport_features,
                )
            else:
                moved, _, _, residual = rigid_procrustes_transport(
                    self.transported_class_means,
                    old_transport_features,
                    current_transport_features,
                )
            drift_norms = (moved - self.transported_class_means).norm(dim=1)
            support_counts = torch.full(
                (known,), old_transport_features.shape[0], dtype=torch.long
            )
            result_prototypes = moved
        else:
            ...
            result_prototypes = result.prototypes
            drift_norms = result.drift_norms
            support_counts = result.support_counts
            residual = None
        self.transported_class_means = torch.cat(
            [result_prototypes.cpu(), new_prototypes], dim=0
        )
        self._last_prototype_transport_stats = {
            "enabled": True,
            "mode": str(options["mode"]),
            "sigma": float(options["sigma"]),
            "old_prototype_count": known,
            "new_prototype_count": int(new_prototypes.shape[0]),
            "mean_drift_norm": float(drift_norms.mean().item()),
            "min_drift_norm": float(drift_norms.min().item()),
            "max_drift_norm": float(drift_norms.max().item()),
            "mean_support_count": float(
                support_counts.float().mean().item()
            ),
            "procrustes_residual": residual,
            "tree": None if tree is None else tree.state_dict(),
        }
        return copy.deepcopy(self._last_prototype_transport_stats)
    ...
    def _build_evaluation_state(self, session_id: int) -> None:
        ...
        if self.method == "create":
            ...
            if self.prototype_transport_options.get("enabled", False):
                old = self.protocol.session(session_id).start
                if self.transported_class_means is None:
                    raise RuntimeError("evaluation lacks transported prototypes")
                expected = self.protocol.session(session_id).stop
                if self.transported_class_means.shape[0] != expected:
                    raise RuntimeError(
                        "transported prototype count does not match seen classes"
                    )
                if old > 0:
                    self.class_means = torch.cat(
                        [
                            self.transported_class_means[:old],
                            self.class_means[old:],
                        ],
                        dim=0,
                    )
        if self.geometry_mode != "none":
    ...
    def _save_checkpoint(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            ...
            "transported_class_means": self.transported_class_means,
            "session_record": (
                None
