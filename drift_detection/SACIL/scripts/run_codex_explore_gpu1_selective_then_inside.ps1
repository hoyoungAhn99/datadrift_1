...
$QueueLog = Join-Path $ProjectRoot "outputs\runner_logs\codex_explore_gpu1_selective_inside.log"
...
function Run-Screen([string]$Config, [string]$RunName, [string]$Group) {
    Assert-FrozenSource
    $metrics = Join-Path $ProjectRoot "outputs\explore\$Group\$RunName\seed_1\metrics.json"
...
Write-QueueLog "GPU1 selective/inside queue initialized"

Run-Screen `
    "configs\explore\cifar100\icarl_htpl_selective_kd_t0.yaml" `
    "screen_s2_icarl_htpl_selective_kd_t0_live2_20260804" `
    "selective_kd"

Run-Screen `
    "configs\explore\cifar100\icarl_edgecorr_inside_r20_lambda05.yaml" `
    "screen_s2_icarl_edgecorr_inside_r20_l05_live_20260804" `
    "edge_correlation"

Write-QueueLog "GPU1 selective/inside queue finished"
