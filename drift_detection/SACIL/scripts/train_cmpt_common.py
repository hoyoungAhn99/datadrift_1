...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a CMPT common-recipe CIL trajectory with the isolated "
            "src_cmpt implementation"
        )
    )
    parser.add_argument("config", type=Path, help="common-recipe YAML config")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "reuse a compatible unified S0 checkpoint; intended for the "
...
def main() -> int:
    ...
    if args.dry_run:
        result = {
            "status": "validated",
            "framework": "sacil-cmpt-common-recipe",
