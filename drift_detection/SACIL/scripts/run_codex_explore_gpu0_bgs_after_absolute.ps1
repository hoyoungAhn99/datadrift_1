...
$QueueLog = Join-Path $ProjectRoot "outputs\runner_logs\codex_explore_gpu0_bgs_queue.log"
...
Write-QueueLog "queue initialized; waiting for GPU0 PID $CurrentGpuPid"
while (Get-Process -Id $CurrentGpuPid -ErrorAction SilentlyContinue) {
    Start-Sleep -Seconds 15
}

Run-Screen `
    "configs\explore\cifar100\icarl_bgs_a4.yaml" `
    "screen_s2_icarl_bgs_a4_frozen_20260804" `
    "boundary_graph_surgery"

Run-Screen `
    "configs\explore\cifar100\icarl_bgs_a7.yaml" `
    "screen_s2_icarl_bgs_a7_frozen_20260804" `
    "boundary_graph_surgery"

Run-Screen `
    "configs\explore\cifar100\icarl_htpl_selective_kd_t0.yaml" `
    "screen_s2_icarl_htpl_selective_kd_t0_frozen_20260804" `
    "selective_kd"

Run-Screen `
    "configs\explore\cifar100\icarl_absolute_local_lambda4.yaml" `
    "screen_s2_icarl_absolute_local_l4_frozen_20260804" `
    "absolute_relaxation"

Write-QueueLog "GPU0 BGS queue finished"
