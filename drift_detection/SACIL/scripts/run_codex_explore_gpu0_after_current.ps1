...
$QueueLog = Join-Path $ProjectRoot "outputs\runner_logs\codex_explore_gpu0_queue.log"
...
Write-QueueLog "queue initialized; waiting for current GPU0 PID $CurrentGpuPid"
while (Get-Process -Id $CurrentGpuPid -ErrorAction SilentlyContinue) {
    Start-Sleep -Seconds 15
}

Run-Screen `
    "configs\explore\cifar100\icarl_absolute_local_lambda16.yaml" `
    "screen_s2_icarl_absolute_local_l16" `
    "absolute_relaxation"

Write-QueueLog "GPU0 queue finished"
