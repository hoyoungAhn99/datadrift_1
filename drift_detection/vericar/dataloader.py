...
class VehicleDataModule(pl.LightningDataModule):
    def __init__(
        self, json_paths, image_path, batch_size=64, num_workers=4, seed=42, flip_p=0.5
    ):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if cache_path.exists():
            ...
            for json_path in tqdm.tqdm(json_files, desc="Processing JSONs"):
                try:
                    ...
                    pt_path = Path(
                        f"{car_type}/{make}/{model}/{year}/{Path(json_path).stem}"
                    ).with_suffix(".pt")
            ...
            # string -> idx
            self.type_to_idx = {
                label: idx for idx, label in enumerate(sorted(type_labels))
            }
            self.make_to_idx = {
                label: idx for idx, label in enumerate(sorted(make_labels))
            }
            self.model_to_idx = {
                label: idx for idx, label in enumerate(sorted(model_labels))
            }
            self.year_to_idx = {
                label: idx for idx, label in enumerate(sorted(year_labels))
            }
            self.all_labels_to_idx = {
                label: idx for idx, label in enumerate(sorted(all_labels))
            }
            ...
            self.idx_to_all_labels = {
                idx: label for label, idx in self.all_labels_to_idx.items()
            }
        ...
        print(
            f"Found {len(self.metadata)} images and {len(self.all_labels_to_idx)} unique all_labels classes."
        )
