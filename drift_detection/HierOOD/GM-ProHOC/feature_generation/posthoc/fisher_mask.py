...
from feature_generation.utils.statistics import fisher_scores


class FisherMaskFeatureGenerator(BaseFeatureGenerator):
    name = "fisher_mask"
    ...
    def __init__(self, num_depths: int, config: dict[str, Any]):
        ...
        self.eps = float(self.mask_cfg.get("eps", 1e-6))
        self.scores: dict[int, torch.Tensor] = {}
        self.masks: dict[int, torch.Tensor] = {}

    def fit(self, train_features: torch.Tensor, labels_by_depth=None, metadata=None) -> "FisherMaskFeatureGenerator":
        del metadata
        if labels_by_depth is None:
            raise ValueError("fisher_mask requires labels_by_depth")
        feats = train_features.float().cpu()
        for depth in range(1, self.num_depths + 1):
            scores = fisher_scores(feats, labels_by_depth[depth].long().cpu(), eps=self.eps)
    ...
    def load_state_dict(self, state: dict[str, Any]) -> "FisherMaskFeatureGenerator":
